...
def _classify_kind(path: Path) -> str:
    ...
    if suffix == ".rs":
        return "rust"
    return "text" if suffix in {".sample", ".txt", ""} else "binary"
...
def summarize_path(root: Path, path: Path, kind: str) -> str:
    ...
    if relative == MARKDOWN_OUTPUT_PATH.as_posix():
        return "Generated Markdown inventory of tracked repository files"
    if relative == CSV_OUTPUT_PATH.as_posix():
        return "Generated CSV inventory of tracked repository files"
    if relative == "docs/audit/README.md":
        return "Audit index for dated repository verification notes"
...
def render_markdown(rows: tuple[FileSummaryRow, ...]) -> str:
    ...
    lines = [
        (
            "<!-- Generated by uv run repo-rag sync-file-summaries --root . "
