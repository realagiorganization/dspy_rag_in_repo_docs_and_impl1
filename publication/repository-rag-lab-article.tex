...
\begin{itemize}
  ...
  \item a dedicated DSPy guide in \texttt{docs/architecture/dspy-rag-guide.md}
\end{itemize}
...
The project still has visible limits, and the repository now documents them through one shared
backlog source. The Markdown backlog and the publication table below are generated from the same
\texttt{todo-backlog.yaml} file so the working roadmap is visible in both the repository and the
article.

\input{todo-backlog-table.tex}
