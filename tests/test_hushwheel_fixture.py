...
def test_hushwheel_fixture_is_large_and_c_indexable() -> None:
    ...
    assert Path("docs/constellation-atlas.md") in indexed_paths
...
def test_hushwheel_fixture_manifest_and_atlas_are_consolidated() -> None:
    manifest = json.loads((FIXTURE_ROOT / "fixture-manifest.json").read_text(encoding="utf-8"))
    atlas = (FIXTURE_ROOT / "docs" / "constellation-atlas.md").read_text(encoding="utf-8")

    assert "docs/constellation-atlas.md" in manifest["doc_files"]
    assert "quality" in manifest["quality_targets"]
    assert "hardening" in manifest["quality_targets"]
    assert atlas.count("## Orbit ") == 6
    assert atlas.count("### Signal") == 6
    assert atlas.count("### Counter-Signal") == 6
    assert atlas.count("### Retrieval Cue") == 6
    assert atlas.count("### Ledger Note") == 6
